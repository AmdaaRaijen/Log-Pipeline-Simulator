...
export default function MultiResultPanel({
  result,
  error,
  isLoading,
  ...
  return (
    <div className="h-full flex flex-col bg-gray-900 text-gray-200 overflow-y-auto">
      ...
      <div className="p-4 border-b border-gray-800 bg-gray-950 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <span className="font-medium text-sm">
            1 Input Event &rarr;{" "}
            <strong className="text-white">{finalEvents.length}</strong> Output
            Events
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left pane: Trace */}
        <div className="w-1/3 flex flex-col border-r border-gray-800 p-4 overflow-y-auto">
          <h3 className="text-sm font-semibold mb-3 uppercase tracking-wider text-gray-500">
            Evaluation Trace (Sequential)
          </h3>
          <div className="space-y-2">
            {trace.map((t, idx) => (
              <div
                key={idx}
                className="border border-gray-700 rounded-md bg-gray-800 overflow-hidden text-sm"
              >
                <button
                  onClick={() =>
                    setExpandedTrace(expandedTrace === idx ? null : idx)
                  }
                ...
                >
                  <div className="flex items-center justify-between w-full">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="bg-gray-700 px-2 rounded-full text-xs text-blue-300">
                        {t.plugin}
                      </span>
                    </div>
                    {expandedTrace === idx ? (
            ...
            {trace.length === 0 && (
              <div className="text-gray-500 text-sm">No rules executed.</div>
            )}
          </div>
        </div>
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="flex-1 bg-[#1e1e1e] p-4 rounded-md border border-gray-700 overflow-y-auto text-sm">
                {finalEvents[activeEventIdx] && (
                  <>
                    <div className="mb-4">
                      <h4 className="text-xs uppercase text-gray-500 font-bold mb-1 border-b border-gray-700 pb-1">
                        Lineage / ID
                      </h4>
                      <div className="text-gray-400 font-mono text-xs">
                        ID: {finalEvents[activeEventIdx].__id}
                        {finalEvents[activeEventIdx].__parentId && (
                          <div>
                            Parent: {finalEvents[activeEventIdx].__parentId}
                          </div>
                        )}
                      </div>
                    </div>

                    {Object.keys(finalEvents[activeEventIdx].metadata).length >
                      0 && (
                      <div className="mb-4">
                        <h4 className="text-xs uppercase text-blue-400 font-bold mb-1 border-b border-gray-700 pb-1">
                          @metadata (Internal Only)
                        </h4>
                        <pre className="text-blue-300 overflow-x-auto text-xs">
                          {JSON.stringify(
                            finalEvents[activeEventIdx].metadata,
                            null,
                            2,
                          )}
                        </pre>
                    ...
                    <div>
                      <h4 className="text-xs uppercase text-green-500 font-bold mb-1 border-b border-gray-700 pb-1">
                        Body (Elasticsearch Document)
                      </h4>
                      <pre className="text-green-300 overflow-x-auto">
                        {JSON.stringify(
                          finalEvents[activeEventIdx].body,
                          null,
                          2,
                        )}
                      </pre>
